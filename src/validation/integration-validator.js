...
class IntegrationValidator {
    ...
    async validateWorkflowIntegrity() {
        try {
            ...
            const configManager = require('../config/configuration-manager');
            if (configManager) {
                this.validationResults.workflowIntegrity.push({
                    test: 'Configuration Loading',
                    status: 'PASS',
            ...
            const orchestratorPath = path.join(__dirname, '..', 'workflows', 'documentation-orchestrator.js');
    ...
    async validateContentRulesImplementation(contentRules) {
        const builderPath = path.join(__dirname, '..', 'content-builders');
    ...
    async validateValidationRulesImplementation(validationRules) {
        const controllerPath = path.join(__dirname, '..', 'quality-controllers');
    ...
    async validateSourceUrlConsistency(sources) {
        const dataExtractorPath = path.join(__dirname, '..', 'data-extractors');
